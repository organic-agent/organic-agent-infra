...
- **embedder·score는 갤러리를 샤드로 나눠 동시에 돈다.** wes가 부른 실행은 조정자가 되어 사진 250장당 샤드 하나(최대 8)로 자기 함수를 다시 EVENT 하고 끝난다. 샤드는 자기 몫만 처리한다. embedder 샤드는 각자 끝나면 그만이고(앱이 `photo_analysis`를 세어 단계를 닫는다), score는 마지막으로 끝난 샤드가 `wes-categorize`를 부른다. 그래서 두 함수의 예약 동시성은 샤드 상한(8) 이상이어야 한다 — 낮으면 샤드가 스로틀되어 라운드가 늘어난다.
...
|---|---|---|---|---|---|
| `wes-embedder` | 3GB | 512MB | 8 (= 샤드 상한) | `embedder` | S3 원본 읽기·`previews/` 쓰기, 자기 재호출(샤드 fan-out) |
